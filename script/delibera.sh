...
#   Copyright (C) 2014 Daniela Soares Feitosa <daniela@colivre.coop.br>
#                      Gardênia Nogueira Lima <garnoli@gmail.com>
#   Copyright (C) 2013 Valessio Brito <contato@valessiobrito.com.br>
#                      Luciano Santa Brígida <lucianosb@sbvirtual.com.br>
#
#   Este arquivo é parte do programa Delibera Mulheres. Este programa é baseado
#   no Deliberatório, que é um
...
CARD_GOALS="$PWD/templates/$CARD_TEMPLATE/goal.svg"
...
mkdir -p $PWD/new_cards

CARDS="EVENTS PAUTAS CARDDEP CARDORG GOALS"
...
"
    ...
    for lineItem in `seq 1 $(cat $CARD_FILE | wc -l)`; do
        ...
        FILE_ITEM="$PWD/new_cards/$(echo $cardItem)_$lineItem"
...
"
inkscape -z $PWD/templates/instrucoes-delibera.svg -A $PWD/templates/instrucoes-delibera.pdf 2> /dev/null > /dev/null
...
" | sed s%$PWD%%g)
AUTORES="FEITOSA, Daniela S., LIMA, Gardênia Nogueira, BRÍGIDA, Luciano S. BRITO, Valessio S."
TERMOS="jogo, cartas, politica, câmara deputados, mulheres, gênero"

pdfjoin --paper a4paper --frame true --pdftitle "Delibera Mulheres" --pdfauthor "$AUTORES" --pdfsubject "$(echo $DESC)" --pdfkeywords "$TERMOS" --nup 3x3 $PWD/new_cards/*.pdf -o $PWD/cards_9xA4.pdf 2> /dev/null
pdfjoin $PWD/templates/instrucoes-delibera.pdf $PWD/cards_9xA4.pdf -o Deliberatorio_9xA4_$(date +%d%m%Y).pdf 2> /dev/null > /dev/null
pdfjoin --paper a4paper --frame true --pdftitle "Deliberatório" --pdfauthor "$AUTORES" --pdfsubject "$(echo $DESC)" --pdfkeywords
"$TERMOS" --nup 4x4 $PWD/new_cards/*.pdf -o $PWD/cards_16xA4.pdf 2> /dev/null
pdfjoin $PWD/templates/instrucoes-delibera.pdf $PWD/cards_9xA4.pdf -o Deliberatorio_16xA4_$(date +%d%m%Y).pdf 2> /dev/null > /dev/null
rm templates/instrucoes-delibera.pdf cards_9xA4.pdf cards_16xA4.pdf 2> /dev/null > /dev/null
